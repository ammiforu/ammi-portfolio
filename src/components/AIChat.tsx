...
export const AIChat: React.FC = () => {
  ...
  return (
    <>
      {/* Floating Highlight Nudge & Arrow */}
      <AnimatePresence>
        {!open && showNudge && (
          <motion.div
            ...
            className="fixed bottom-24 right-6 z-50 flex flex-col items-end pointer-events-auto"
          >
            <div
              onClick={() => setOpen(true)}
              className="cursor-pointer group flex items-center gap-2.5 pl-3.5 pr-2 py-2 rounded-2xl bg-[#0c0d18]/95 border border-[#00f0ff]/40 shadow-[0_4px_30px_rgba(0,240,255,0.25)] backdrop-blur-md hover:border-[#00f0ff] hover:shadow-[0_4px_35px_rgba(0,240,255,0.45)] transition-all duration-300"
            >
              ...
              <div className="text-left">
                <div className="text-xs font-bold font-syne text-white flex items-center gap-1.5 leading-tight">
                  <span>Want to know about me?</span>
                  <span className="text-[#00f0ff]">Click here!</span>
                </div>
                <div className="text-[10px] font-mono text-[#9496a8] leading-tight">
                  Instant AI Answers ⚡
                </div>
              </div>
              <button
                ...
                className="ml-1 p-1 rounded-lg text-[#9496a8] hover:text-white hover:bg-white/10 transition-colors"
                title="Dismiss"
            ...
            {/* Bouncing Pointer Arrow toward chatbot button */}
            <motion.div
              animate={{ y: [0, 5, 0] }}
              transition={{ repeat: Infinity, duration: 1.2, ease: "easeInOut" }}
              className="mr-6 mt-1 flex flex-col items-center"
            >
              <div className="w-0.5 h-2.5 bg-gradient-to-b from-[#00f0ff]/50 to-[#00f0ff]" />
              <div className="w-0 h-0 border-l-[5px] border-l-transparent border-r-[5px] border-r-transparent border-t-[7px] border-t-[#00f0ff]" />
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="fixed bottom-6 right-6 z-50 group">
        {/* Glow halo */}
        <div className="absolute -inset-1 rounded-full bg-gradient-to-r from-[#00f0ff] via-[#3b82f6] to-[#00f0ff] opacity-70 blur-md animate-pulse group-hover:opacity-100 transition-opacity pointer-events-none" />

        <motion.button
          onClick={() => setOpen((o) => !o)}
          whileHover={{ scale: 1.08 }}
          whileTap={{ scale: 0.95 }}
          className="relative w-14 h-14 rounded-full bg-gradient-to-br from-[#00f0ff] to-[#3b82f6] shadow-[0_0_24px_rgba(0,240,255,0.5)] flex items-center justify-center text-[#050811] transition-all duration-300"
        ...
        >
          ...
          {/* Active online dot on launcher */}
          {!open && (
            <span className="absolute top-0 right-0 flex h-3.5 w-3.5 -mt-0.5 -mr-0.5">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-3.5 w-3.5 bg-emerald-400 border-2 border-[#0d0d14]"></span>
            </span>
          )}
        </motion.button>
      </div>
